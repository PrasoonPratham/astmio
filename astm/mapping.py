...
class Field(object):
    ...
    def _set_value(self, value):
        ...
        if self.length is not None and len(value) > self.length:
            raise ValueError(
                "Field %r value is too long (max %d, got %d)"
...
class Mapping(metaclass=MetaMapping):
    ...
    def __repr__(self):
        return "%s(%s)" % (
            ...
            ", ".join("%s=%r" % (key, value) for key, value in self.items()),
        )
...
class ConstantField(Field):
    ...
    def _set_value(self, value):
        ...
        if self.default != value:
            raise ValueError(
                "Field changing not allowed: got %r, accepts %r" % (value, self.default)
            )
    ...
    def _get_value(self, value):
        if isinstance(value, str):
            try:
                ...
                raise ValueError(
                    "Value %r does not match format %s" % (value, self.format)
                )
        return value
...
class RepeatedComponentField(Field):
    ...
    class Proxy(list):
        def __init__(self, seq, field):
            super(RepeatedComponentField.Proxy, self).__init__(
                field._get_value(i) for i in seq
            )
            self.field = field

        def __setitem__(self, index, value):
            if isinstance(index, slice):
                super(RepeatedComponentField.Proxy, self).__setitem__(
                    index, [self.field._set_value(v) for v in value]
                )
            else:
                super(RepeatedComponentField.Proxy, self).__setitem__(
                    index, self.field._set_value(value)
                )

        def append(self, item):
            super(RepeatedComponentField.Proxy, self).append(
                self.field._set_value(item)
            )

        def extend(self, other):
            super(RepeatedComponentField.Proxy, self).extend(
                self.field._set_value(i) for i in other
            )
...
class NotUsedField(Field):
    ...
    def _set_value(self, value):
        warnings.warn(
            "Field %r is not used, any assignments are omitted" % self.name, UserWarning
        )
        return None
