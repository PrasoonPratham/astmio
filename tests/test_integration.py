...
from astmio import (
    ...
    decode, decode_record, encode_message, 
    send_astm_data, run_astm_server
)
from astmio.constants import STX, ETX, ETB, EOT, ENQ, ACK, NAK, CR, LF, CRLF, RECORD_SEP
...
class TestIntegration:
    ...
    async def test_high_level_send_receive(self, simple_handlers):
        ...
        # Start server
        async with astm_server(handlers, timeout=3.0) as server:
            # Create test message
            records = [
                ['H', ['', '^&'], None, None, 'Test Client', None, None, None, 
                 None, None, None, 'P', 'E1394-97', '20250701'],
                ['P', '1'],
    ...
    async def test_message_framing(self):
        ...
        records = [
            ['H', ['', '^&'], None, None, 'Test Lab', None, None, None,
             None, None, None, 'P', 'E1394-97', '20250701'],
    ...
    async def test_multi_frame_message(self):
        ...
        try:
            ...
            records = [
                ...
                ['O', '1', 'SAMPLE001', None, '^^^TEST1\\^^^TEST2\\^^^TEST3\\^^^TEST4\\^^^TEST5']
            ]
            
            # Add many result records
            for i in range(20):
                records.append(['R', str(i+1), f'^^^TEST{i+1}', '100', 'units', '50-150', 'N'])
    ...
    def test_real_world_message_parsing(self):
        ...
        # This is a raw message without STX/ETX framing, so decode should return one record
        # (the whole message as one record). We need to manually split by RECORD_SEP
    ...
    async def test_server_duration_limit(self, simple_handlers):
        ...
        assert 0.8 <= duration <= 1.5, f"Server should run for ~1 second, ran for {duration}"
