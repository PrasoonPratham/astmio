...
astmio.install("hipaa", db_path="production_audit.db", retention_days=2555)
astmio.install("metrics")

print(f"✅ Plugins installed: {astmio.list_installed()}")
...
def handle_patient(record, server):
    ...
    if len(record) > 1:
        ...
        if '^' in patient_name:
            ...
            formatted_name = f"{name_parts[1]} {name_parts[0]}" if len(name_parts) >= 2 else patient_name
        else:
    ...
    if hipaa_plugin:
        try:
            ...
            if report and report.get('summary'):
                ...
                print(f"   📊 Today's HIPAA summary: {summary['total_events']} events, "
...
async def main():
    ...
    server = astmio.create_server(
        ...
        profile="etc/profiles/mindray_bs240.yaml" if os.path.exists("etc/profiles/mindray_bs240.yaml") else None,
        log_level="INFO"
    )
    
    print(f"🔌 Server plugins: {server.list_plugins()}")
    print(f"🌐 Server listening on port 15200")
    print(f"📝 Send ASTM data to this server for processing")
