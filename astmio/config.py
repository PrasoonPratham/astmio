...
class TransportConfig:
    ...
    def __post_init__(self):
        ...
        if self.max_connections <= 0:
            raise ValidationError(f"Max connections must be positive: {self.max_connections}")
...
class FrameConfig:
    ...
    def __post_init__(self):
        ...
        if self.chunk_size > self.max_length:
            log.warning(f"Chunk size ({self.chunk_size}) larger than max length ({self.max_length})")
...
class RecordFieldMapping:
    ...
    def __post_init__(self):
        ...
        if self.astm_position < 0:
            raise ValidationError(f"ASTM position must be non-negative: {self.astm_position}")
...
class RecordConfig:
    ...
    def __post_init__(self):
        ...
        if len(positions) != len(set(positions)):
            raise ValidationError(f"Duplicate ASTM positions in record {self.record_type}")
        ...
        try:
            ...
            for field_data in data.get('fields', []):
                if isinstance(field_data, dict):
                    ...
                    fields.append(RecordFieldMapping(field_name=field_data, astm_position=len(fields), required=is_required))
                else:
            ...
            raise ValidationError(f"Invalid record configuration for {record_type}: {e}")
...
class ParserConfig:
    ...
    def __post_init__(self):
        ...
        if self.max_message_size <= 0:
            raise ValidationError(f"Max message size must be positive: {self.max_message_size}")
...
class DeviceProfile:
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any], source_file: Optional[str] = None) -> "DeviceProfile":
        ...
        try:
            ...
            for record_type_str, record_data in records_data.items():
                try:
                    ...
                    records[record_type] = RecordConfig.from_dict(record_type_str, record_data)
                except ValueError:
            ...
            log.info("Loaded profile for %s (vendor: %s, model: %s)", device, profile.vendor, profile.model)
            return profile
    ...
    def save_to_file(self, filepath: Union[str, Path], format: ProfileFormat = ProfileFormat.YAML) -> None:
        ...
        try:
            ...
            for record_type, config in self.records.items():
                try:
                    ...
                    if not required_fields and strict:
                        errors.append(f"Record {record_type.value} has no required fields")
                    
                    # Check field mappings
                    positions = [f.astm_position for f in config.fields]
                    if len(positions) != len(set(positions)):
                        errors.append(f"Duplicate ASTM positions in record {record_type.value}")
            ...
            try:
                if self.parser.max_message_size <= 0:
                    errors.append(f"Invalid max message size: {self.parser.max_message_size}")
            except Exception as e:
    ...
    def merge_with(self, other: 'DeviceProfile', prefer_other: bool = True) -> 'DeviceProfile':
...
class ConfigManager:
    ...
    def add_profile(self, profile: DeviceProfile) -> None:
        ...
        old_profile = self._profiles.get(profile.device)
    ...
    def export_profiles(self, directory: Union[str, Path], format: ProfileFormat = ProfileFormat.YAML) -> None:
        ...
        for device, profile in self._profiles.items():
            # Sanitize filename
            safe_name = "".join(c for c in device if c.isalnum() or c in ('-', '_')).lower()
