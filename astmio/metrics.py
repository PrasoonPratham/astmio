...
class Metric:
    ...
    def __init__(self, name: str, metric_type: MetricType, description: str = "", 
                 max_points: int = 1000, labels: Optional[Dict[str, str]] = None):
        self.name = name
...
class Histogram(Metric):
    ...
    def __init__(self, name: str, description: str = "", 
                 buckets: Optional[List[float]] = None, **kwargs):
...
class MetricsCollector:
    ...
    def _init_astm_metrics(self) -> None:
        ...
        self.register_counter("astm_messages_received_total", "Total ASTM messages received")
        self.register_counter("astm_messages_failed_total", "Total ASTM message failures")
        
        # Connection metrics
        self.register_gauge("astm_active_connections", "Number of active ASTM connections")
        self.register_counter("astm_connections_total", "Total ASTM connections established")
        self.register_counter("astm_connection_errors_total", "Total ASTM connection errors")
        
        # Performance metrics
        self.register_timer("astm_message_processing_duration", "Time to process ASTM messages")
        self.register_timer("astm_connection_duration", "Duration of ASTM connections")
        self.register_histogram("astm_message_size_bytes", "Size of ASTM messages in bytes")
        
        # Error metrics
        self.register_counter("astm_checksum_errors_total", "Total checksum validation errors")
        ...
        for record_type in ['H', 'P', 'O', 'R', 'C', 'L']:
            self.register_counter(f"astm_records_{record_type.lower()}_total", 
        ...
        with self._lock:
            if name in self._metrics:
                ...
                if not isinstance(metric, Counter):
                    raise ValidationError(f"Metric {name} already exists with different type")
                return metric
    ...
    def get_summaries(self, since: Optional[datetime] = None) -> Dict[str, MetricSummary]:
